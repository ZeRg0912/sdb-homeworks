...
CREATE TABLE users_core (
    ...
    CONSTRAINT chk_users_core_shard1 CHECK (user_id BETWEEN 1 AND 1000000)
);
...
CREATE TABLE users_auth (
    ...
    CONSTRAINT chk_users_auth_shard1 CHECK (user_id BETWEEN 1 AND 1000000)
);
...
CREATE TABLE books_core (
    ...
    year INT,
    CONSTRAINT chk_books_core_shard1 CHECK (book_id BETWEEN 1 AND 500000)
);
...
CREATE TABLE books_text (
    book_id BIGINT PRIMARY KEY,
    description TEXT,
    CONSTRAINT chk_books_text_shard1 CHECK (book_id BETWEEN 1 AND 500000)
);
...
CREATE TABLE shops_core (
    ...
    manager VARCHAR(255),
    CONSTRAINT chk_shops_core_shard1 CHECK (shop_id BETWEEN 1 AND 100000)
);
...
CREATE TABLE shops_info (
    ...
    CONSTRAINT chk_shops_info_shard1 CHECK (shop_id BETWEEN 1 AND 100000)
);
